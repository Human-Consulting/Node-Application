...
const FormsEditarSenhaUsuario = ({ id, toggleModal, token, debug }) => {
  // Log de debug para rastrear token recebido
  if (debug) debug();
  if (token) {
    console.log(
      "[RESET][FRONT] FormsEsqueciASenha recebeu token:",
      token,
      "para id:",
      id
    );
  ...
  const [showConfirmarSenhaAtualizada, setShowConfirmarSenhaAtualizada] =
    useState(false);
  ...
  const handlePutUsuario = async () => {
    ...
    // Se vier token, envia junto no body
    const body = token ? { token, senhaAtualizada } : { senhaAtualizada };
    console.log(
      "[RESET][FRONT] Enviando body para putEsqueciASenhaUsuario:",
      body,
      "id:",
      id
    );
    ...
    if (resposta) {
      console.log("[RESET][FRONT] Senha redefinida com sucesso!");
      toggleModal();
      setSenhaAtualizada("");
      setConfirmarSenhaAtualizada("");
    } else {
      console.warn("[RESET][FRONT] Falha ao redefinir senha.");
    }
  };
  ...
  return (
    ...
    >
      ...
      <TextField
        ...
        helperText={
          erroTamanhoSenha ? "A senha deve ter pelo menos 6 dígitos!" : ""
        }
      />
        ...
        InputProps={{
          ...
          endAdornment: (
            <InputAdornment position="end">
              <IconButton
                onClick={() =>
                  setShowConfirmarSenhaAtualizada(!showConfirmarSenhaAtualizada)
                }
                edge="end"
              >
                {showConfirmarSenhaAtualizada ? (
                  <VisibilityOff />
                ) : (
                  <Visibility />
                )}
...
FormsEditarSenhaUsuario.propTypes = {
  id: PropTypes.any,
