...
const ModalProjeto = ({
  ...
  const buscarUsuarios = async (page = 0, nome) => {
    const usuariosRetornados = await getUsuariosResponsaveis(
      Number(fkEmpresa),
      page,
      4,
      nome || null
  ...
  const validarCampos = () => {
    ...
    if (!validarPermissaoFunc() && fkResponsavel === "#")
      novosErros.fkResponsavel = "Responsável é obrigatório";
  ...
  return (
    <Dialog open={open} onClose={handleOnClose} fullWidth maxWidth="xs">
      ...
      <Actions>
        {projeto == null ? (
          <Button variant="contained" onClick={handlePostProjeto} endIcon={<Send />}>
            Adicionar
          </Button>
        ) : (
          <>
