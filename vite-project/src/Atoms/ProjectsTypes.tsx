...
export interface ProjectsTypesEntidade {
...
interface ProjectsTypesProps {
  entidade: ProjectsTypesEntidade;
  diminuirLateralBar?: boolean;
  telaAtual?: () => void;
}

const ProjectsTypes = ({ entidade, diminuirLateralBar }: ProjectsTypesProps) => {
        ...
        {!diminuirLateralBar && (
          <SubTitle>
            {nomeEmpresa == 'Empresas'
              ? `Esta empresa está ${Math.floor(entidade.progresso ?? 0)}% concluída`
              : `Este projeto está ${Math.floor(entidade.progresso ?? 0)}% concluído`}
          </SubTitle>
        )}
      </Stack>
    </ProjectsTypesBox>
